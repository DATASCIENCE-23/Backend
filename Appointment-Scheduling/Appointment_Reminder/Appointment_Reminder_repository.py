...
class AppointmentReminderRepository:
    """Repository layer for Appointment Reminder"""
    ...
    def get_by_id(
        ...
        return (
            ...
            .filter(AppointmentReminder.reminder_id == reminder_id)
            .first()
        )
    ...
    def get_by_status(
        ...
        return (
            db.query(AppointmentReminder)
            .filter(AppointmentReminder.status == status)
            .order_by(AppointmentReminder.reminder_time)
            .all()
        )
    ...
    def get_pending_reminders(
        ...
        return (
            db.query(AppointmentReminder)
            .filter(AppointmentReminder.status == ReminderStatusEnum.PENDING)
            .order_by(AppointmentReminder.reminder_time)
            .all()
        )
    ...
    def get_failed_reminders(
        ...
        return (
            db.query(AppointmentReminder)
            .filter(AppointmentReminder.status == ReminderStatusEnum.FAILED)
            .order_by(desc(AppointmentReminder.reminder_time))
            .all()
        )
    ...
    @staticmethod
    def get_by_reminder_type(
        db: Session,
        reminder_type: ReminderTypeEnum
    ) -> List[AppointmentReminder]:
        return (
            db.query(AppointmentReminder)
            .filter(AppointmentReminder.reminder_type == reminder_type)
            .all()
        )
    ...
    def bulk_update_status(
        ...
        count = (
            db.query(AppointmentReminder)
            .filter(AppointmentReminder.reminder_id.in_(reminder_ids))
            .update(
